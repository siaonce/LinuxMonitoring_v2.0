...
function esponse_code {
    ...
    for (( i=0; i < number_mass; i++ )); do
        for ((j=1; j<=5; j++));do
        awk '$9 == '${arrcods[i]}'  { print $0 }' ../4x/log${j}.log >> sortlog1.log
        done
    done  
}
...
function unique_ips {
    ...
    for ((j=1; j<=5; j++));do
        awk '{print $1}' ../4x/log${j}.log >> iplist.txt
    done
...
function requests {
    ...
    for (( i=0; i < number_mass; i++ )); do
        for ((j=1; j<=5; j++));do
        awk '$9 == '${arrcods[i]}'  { print $9 " " $6 " " $7 $8}' ../4x/log${j}.log >> sortlog3.log
        done
    done  
}
...
function erroneous_requests {
    ...
    for (( i=0; i < number_mass; i++ )); do
        for ((j=1; j<=5; j++));do
        awk '$9 == '${arrcods[i]}'  { print $1 }' ../4x/log${j}.log >> sortlog.txt
        done
    done  
